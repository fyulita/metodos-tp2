...
*/
std::pair<double, Vector> power_iteration(const Matrix& mat, unsigned num_iter=5000, double epsilon=1e-6);
...
pair<Vector, Matrix> donde:
    ...
    - El segundo elemento es una matriz cuyas columnas son los autovectores
      correspondientes
*/
std::pair<Eigen::VectorXd, Matrix> get_first_eigenvalues(const Matrix& mat, unsigned num, unsigned num_iter=5000, double epsilon=1e-6);
